...
#define _DEBUG
    ...
    private:
        ...
        //dot to pallet
        std::map<std::pair<std::pair<unsigned char, unsigned char>, unsigned char>, unsigned char>
            dot_to_plt;
    ...
    public:
        ...
        void write(const wchar_t* const path) const{
        ...
        //パレットを取得する
        struct getplt_BitCountError : BitCountError{//色数例外
            virtual const char *prfn() const{ return "getplt"; }
        };

        struct getplt_OOR : logic_error{//範囲外アクセス
            virtual const char *msg() const{ return "パレットのインデックスが範囲を超えました。"; }
            virtual const char *prfn() const{ return "getplt"; }
        };
        ...
        dot getplt(int i){
#ifdef _DEBUG
            if(BitCount_ > ColorNum::n256) throw(getplt_BitCountError());
            if(i > ((1 << BitCount_) - 1)) throw(getplt_OOR());
#endif //_DEBUG
            return dot_(ColorPalette_[i]);
        }
    ...
    private:
        ...
        dot clr_pltclr(int w, int h, dot d){
            auto iter = dot_to_plt.find(std::make_pair(std::make_pair(d.r, d.g), d.b));
            if(iter == dot_to_plt.end()){
                throw(clr_set_BitCountError());
            }
            unsigned char i = pltidx(w, h);
            pltidx(w, h, iter->second);
            return getplt(i);
        }
...
#define CommonMemberInit \
    ...
    private:
        ...
        static void ctor_read(xbmp &obj, std::basic_ifstream<T, U> &f){
            ...
            if(obj.BitCount_ == ColorNum::n2 || obj.BitCount_ == ColorNum::n16 || obj.BitCount_ == ColorNum::n256){
                int bc = static_cast<int>(obj.BitCount_);
                for(int i = 0; i < (1 << bc); ++i){
                    dot d = obj.getplt(i);
                    obj.dot_to_plt[std::make_pair(std::make_pair(d.r, d.g), d.b)] = static_cast<unsigned char>(i);
                }
            }
        }
    ...
    public:
        ...
        xbmp(const wchar_t *const path) : CommonMemberInit{
